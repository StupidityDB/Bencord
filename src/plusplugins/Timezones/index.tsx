...
export default definePlugin({
  ...
  patches: [
    {
      ...
      replacement: {
        match:
          /(function .+?\(.+?\){let{inPopout:.+allowIdle.+?}=.+?\.usePreventIdle\)\("popup"\),(.+?)=\[\];if\(.+?\){.+"chat-spacer"\)\)\),\(\d,.+?\.jsx\)\(.+?,{children:).+?}}/,
  ...
  settingsAboutComponent() {
    ...
    const hasQuestsExtensionEnabled =
      localStorage.getItem("QUESTS_EXT_ENABLED");

    return (
      <>
        {isDesktop || hasQuestsExtensionEnabled ? (
          <Text variant="text-lg/bold">
            The plugin should work properly because you{" "}
            {isDesktop
              ? "are on the Desktop Client."
              : "installed our extension."}
          </Text>
        ) : (
          <Text variant="text-lg/bold">
  ...
  renderQuestButton() {
    const currentStream: Stream | null = findByProps(
    ...
    if (currentStream) {
      if (
        !findByProps("getParticipants")
          .getParticipants(currentStream.channelId)
          .filter((participent) => participent.user.id !== window.currentUserId)
          .length
      ) {
        shouldDisable = true;
      }
  ...
  openCompleteQuestUI() {
    ...
    const encodedStreamKey =
      findByProps("encodeStreamKey").encodeStreamKey(currentStream);
    quest = getLeftQuests();
    ImagesConfig = {
      icon: findByProps("getQuestBarHeroAssetUrl").getQuestBarHeroAssetUrl(
        quest
      ),
    ...
    const heartBeat = async () => {
      ...
      const res = findByProps("sendHeartbeat").sendHeartbeat({
        questId: quest.id,
  ...
  flux: {
    STREAM_STOP: (event) => {
      const stream: Stream = findByProps("encodeStreamKey").decodeStreamKey(
        event.streamKey
      );
    ...
    QUESTS_SEND_HEARTBEAT_SUCCESS: (event) => {
      ...
      showNotification({
        ...
        body: `Current progress: ${Math.floor(a / b)}% (${Math.floor(
          event.userStatus.streamProgressSeconds / 60
        )} minutes.)`,
        ...ImagesConfig,
      });
